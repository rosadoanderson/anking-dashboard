...
async function boot() {
  ...
  const baseParts = await Promise.all([
    fetch("cards_part1.json").then(r => r.json()),
    fetch("cards_part2.json").then(r => r.json()),
    fetch("cards_part3.json").then(r => r.json()),
    fetch("cards_part4.json").then(r => r.json())
  ]);
...
function resolveMediaIn(container) {
  ...
  container.querySelectorAll("img").forEach(img => {
    ...
    img.loading = "lazy";
    img.decoding = "async";
    resolveMediaPath(original, (path) => { img.src = path; }, () => { img.alt = "Imagem não encontrada"; img.classList.add("media-missing"); });
  });
  container.querySelectorAll("audio").forEach(audio => {
    const original = audio.getAttribute("src") || "";
    if (!original.startsWith("media/")) return;
    const normalized = normalizeMediaName(original);
    if (MEDIA_PATH_CACHE.has(normalized)) { audio.src = MEDIA_PATH_CACHE.get(normalized); audio.load(); return; }
    const candidates = mediaCandidates(normalized);
    let idx = 0;
    audio.preload = "none";
    audio.onerror = () => {
      idx += 1;
      if (idx < candidates.length) { audio.src = candidates[idx]; audio.load(); }
      else audio.classList.add("media-missing");
    };
    audio.onloadeddata = () => { MEDIA_PATH_CACHE.set(normalized, candidates[idx]); };
